...
def main(T, dt):
    ...
    pathlib.Path(data_path, 'features', 'T={},dt={}'.format(T, dt)) \
           .mkdir(parents=True, exist_ok=True)
    ...
    population = pd.read_csv(data_path + 'pop.csv').set_index('ID')[[]]

    # Load in raw data (from prepare.py)
    data = extract_data.stack_attr_columns()
    ...
    df_data.to_pickle(data_path + 'features/input_data.p')
